...
- **笔记与链接**（可直接用于公众号或内部分享）：
  - **北大王鹤团队 2025 盘点**：从扫描版原文 OCR 后整理成文，人物简介 + 全年顶会成果一览（BODex、DexVLG、DyWA、DreamVLA、NavFoM、TrackVLA++、UrbanVLA 等），结构清晰、带论文与项目链接，适合做「学界之星」类推文或人物/团队介绍。（`files/interviews/PKU-WangHe-team-2025-embodied-roundup-笔记.md`）
  - **NVIDIA CHIP**：告别「笨手笨脚」的即插即用柔顺力控模块，不碰参考轨迹、只微调观测目标，双策略（局部/全局 3 点跟踪）+ VR 遥操作 + 多机协同，实测数据与局限一并归纳，适合做技术解读或产品向推文。（`files/papers/NVIDIA-CHIP-humanoid-motor-skills-笔记.md`）
  - **NVIDIA DreamDojo（30 人联名）**：44,000 小时人类视频预训练世界模型、潜在动作跨身体迁移、三步训练与蒸馏实时化，效果与策略评估相关系数 0.995 等亮点提炼，适合做「从人类视频到机器人想象」类深度稿。（`files/papers/NVIDIA-30auth-44k-hours-video-dexterous-manipulation-笔记.md`）
  - 上述三篇均在 `docs/8-people` 对谈表、`docs/5-sota` / `1.4-运控` / `4-classical` 延伸区做了入口链接，方便从综述或人物页跳转阅读与二次传播。
...
- docs 延伸链接集中指向上述笔记，从综述或人物页即可跳转阅读；整理稿结构清晰、带原文与项目链接，便于公众号选用、内部分享或二次传播。
