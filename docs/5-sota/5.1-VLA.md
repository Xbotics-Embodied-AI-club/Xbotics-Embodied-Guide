...
- **Xiaomi-Robotics-0**（小米首代具身 VLA，全面开源）  
  [整理稿](../../files/source/xiaomi-vla-open-source-2026.md) | [索引](../../files/papers/xiaomi-vla-open-source-2026-索引.md)
  
  **创新点**：4.7B 参数，推理延迟约 80ms，30Hz 控制，单卡 RTX 4090 实时；VLM「大脑」+ 16 层 DiT「小脑」、Action Chunk、流匹配、KV Cache 复用；Action Proposal 对齐视觉与动作空间，两阶段训练；Clean Action Prefix + Λ-shape Attention Mask 缓解动作惯性。LIBERO/CALVIN/SimplerEnv 等超越约 30 个模型。
  
  **适用场景**：**通用具身操作**、叠毛巾/拆乐高等真机任务；开源基座便于二次开发与产业化。

- **星海图 G0 系列**（端侧 VLA：G0 Tiny / G0 Plus 开源）  
  [整理稿](../../files/source/galaxea-g0-tiny-vla-2026.md) | [索引](../../files/papers/galaxea-g0-tiny-vla-2026-索引.md)  
  **要点**：G0 Plus 3B 权重与代码开源、高泛化操作；G0 Tiny 250M、NVIDIA Orin 端侧 10Hz 实时、R1 Pro 约 5 分钟部署；后训练框架支持 G0/pi0 等。开源：OpenGalaxea/GalaxeaVLA。

- **VAM 视频动作模型**（VLA 后新趋势：6 篇汇总）  
  [整理稿](../../files/source/vam-video-action-model-survey-2026.md) | [索引](../../files/papers/vam-video-action-model-survey-2026-索引.md)  
  **要点**：用视频生成模型做机器人控制（next physical state prediction）；两流派（先想象再模仿 vs 借用大脑）；Video Policy、LVP、mimic-video、1XWM、Cosmos Policy、DreamZero 对比与局限。
